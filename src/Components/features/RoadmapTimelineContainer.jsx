...
const RoadmapTimelineContainer = ({ activeId, onSelect }) => {
  return (
    <div className="relative flex flex-col items-center gap-12">
      ...
      <div className="absolute top-10 bottom-10 left-1/2 w-0.5 bg-gray-900 -translate-x-1/2"></div>

      {/* 2. Karena roadmapData adalah Array, kita bisa langsung melakukan mapping */}
      {roadmapData.map((phase) => (
        <button 
          key={phase.id} 
          className={`relative z-10 flex items-center justify-center p-3 text-white rounded-lg min-w-[120px] shadow-md transition-all duration-500 ease-in-out hover:scale-105 ${activeId === phase.id ? 'bg-slate-800 ring-4 ring-slate-100' : 'bg-slate-600 hover:bg-slate-700'}`}
        ...
        >
          ...
          <h4 className="font-semibold text-sm">{phase.quarter}</h4>
        </button>
      ))}
    </div>
  );
